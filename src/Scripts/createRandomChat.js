...
// Query random-chat-requests collection for two documents
async function getTwoRandomChatRequests() {
    const snapshot = await db.collection('random-chat-request').limit(2).get();
...
// create new collection {user0.user1}

// add collection name to both users' documents

getTwoRandomChatRequests()